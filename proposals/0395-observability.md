...
* Version 5: Converts `Observable` to a regular (non-marker) protocol, limits the `@Observable` macro to classes, and other clarifications.
...
The `@Observable` macro implements conformance to the `Observable` protocol and tracking for each stored property. Unlike `ObservableObject` and `@Published`, the properties of an `@Observable` type do not need to be individually marked as observable. Instead, all stored properties are implicitly observable.
...
Observable types conform to the `Observable` protocol. While the `Observable` protocol doesn't have any formal requirements, it includes a semantic requirement that conforming types must implement tracking for each stored property using an `ObservationRegistrar`. Most types can meet that requirement simply by using the `@Observable` macro:
...
extension Model: Observable {}
```

The macro is designed to fill in the gaps in a type's `Observable` implementation.
Any methods that the macro generates are skipped if already present in the `@Observable`-attributed type declaration, so a developer can provide their own implementation or copy and modify the macro's initial output. In addition, the conformance extension is omitted when a type already conforms to `Observable`, either through the original type declaration or through subclassing a conforming type.
...
Future work could explore applying the `@ObservationTracked` attribute to computed properties, instead of manually implementing tracking for computed properties that use remote storage. The correct implementation of this feature may depend on additional capabilities of Swift macros.

Another future enhancement could provide access to information about whether an observable type has observers, or a notification of when observation begins or ends.
